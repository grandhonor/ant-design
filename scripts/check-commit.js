...
async function checkVersion() {
  const { versions } = await fetch('http://registry.npmjs.org/gh-antd').then(res => res.json());
...
async function checkRemote() {
  ...
  if (remote.indexOf('grandhonor/ant-design') === -1) {
    console.log(
      chalk.yellow('😓 Your remote origin is not grandhonor/ant-design, did you fork it?'),
    );
    exitProcess();
  }
}
